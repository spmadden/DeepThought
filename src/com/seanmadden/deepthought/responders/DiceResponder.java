...
public class DiceResponder implements MessageHandler {
	...
	public boolean handleMessage(IRCClient irc, Message m) {
		...
		try{
			...
			if(die > 30 || die <= 1){
				Message msg = new Message("", "PRIVMSG", m.getUsermask() + ": No.", m.getTarget());
				irc.sendMessage(msg);
				return true;
			}
			...
			Message msg = new Message("", "PRIVMSG", m.getUsermask() + ": " + response, m.getTarget());
			irc.sendMessage(msg);
		}catch(NumberFormatException e){
			Message msg = new Message("", "PRIVMSG", m.getUsermask() + ": can't figure out what you meant!", m.getTarget());
			irc.sendMessage(msg);
		}
		return true;
	}
